...
class Dynamicwallpaper():
    def __init__(self):
        ...
        self.headers = {
            ...
            'Connection': 'close' #不使用持久连接,防止request的连接数过多而导致Max retries exceeded
        }
        self.page=1
    ...
    def get_ids(self,page_url):
        ...
        # print(r.text)
        ids=re.findall('<a href="/wallpaper/(.+)" class="link"',r.text)
        return ids
    ...
    def get_true_heic_url(self,page_url):
        #三个必要参数
        ids = self.get_ids(page_url) #参数1、val
    ...
    def download(self, path, name, type, href):  #
        ...
        # 发送请求
        try:
            r = requests.get(href,verify=False,  headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.132 Safari/537.36'}) #stream=True,
    ...
    def download_one_page(self , page_url='https://dynamicwallpaper.club/gallery?section=new'):
        print('\n[{}]正在下载第「{}」页:'.format(time.strftime('%H:%M:%S'),self.page),page_url)
        ...
        for i,url in enumerate(true_heic_urls):
            #方式1、直接下载，需连接vpn下载，采取多线程方式下载
            threading.Thread(target=self.download,args=['./heics_test_Thread',
                                                        self.names[i].replace('%20',' '),
                                                        'heic',
                                                        url]).start()
